    ...
    const onSubmit = (data) => {
        if (data) {
            if (error.length === 0) {
                ...
                axios({
                    method: 'POST',
                    url: 'http://localhost:4000/borrowers/set',
                    headers: {
            ...
            <div className={`content ${popUpActive}`}>
                <div className="top">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6" onClick={() => {
                        setPopUpActive('')
                        setTimeout(() => {
                            props.setAddBorrowerPopUp(false)
                        }, 300)
                    }}>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className='input'>
                        ...
                        <select {...register("itemno", { required: true })} onClick={(e) => {
                            console.log(e.currentTarget.value, props.borrowersData)
                            setError(props.borrowersData.filter((data) => parseInt(data.item.id) === parseInt(e.currentTarget.value) && !data.return_status))
                        }}>
                        ...
                        <span>
                            ...
                            {error.map((data) => data.item.name + ' alredy taken please select other items')}
                        </span>
                    </div>
                    <div className='input'>
                        <p> Phone number</p>
