...
function AddItemPopUp(props) {
    ...
    useEffect(() => {
        ...
        setFiltredStock(props.allStockData.filter((data) => data.category !== null))
            ...
            <div className={`content ${popUpActive}`}>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className='input'>
                        ...
                        <span>
                            {errors.name?.type === "minLength" && "Item Name must contain 2 character or number"}
                            {errors.name?.type === "maxLength" && "Item Name must be less than 10 length "}
                            {errors.name?.type === "required" && "Item Name Required"}
                        </span>
                    </div>
                    <div className='input'>
                        <p>Select Stock</p>
                    ...
                    <div className='input'>
                        <p> Warranty</p>
                        <input type='text' {...register("warrantyDate", { required: true, pattern: /^[0-9]{4}[-]{1}[0-9]{2}[-]{1}[0-9]{2}$/g })} placeholder='yyyy-mm-dd' />
                        <span>
                            {errors.warrantyDate?.type === "required" && "Warranty Date Required"}
                            {errors.warrantyDate?.type === "pattern" && "YYYY-MM-DD"}
                        </span>
                    </div>
                    <div className='input'>
                        <p> Item Type</p>
                        <input type='text' {...register("itemType", { required: true, minLength: 1, maxLength: 50 })} />
                        <span>
                            {errors.itemType?.type === "required" && "This field Required"}
                            {errors.itemType?.type === "minLength" && "This field must contain 1 character or number"}
                            {errors.itemType?.type === "maxLength" && "This field must be less than 50 length"}
                        </span>
                    </div>
                    <div className='input'>
                        <p> Lab Location</p>
                    ...
                    <div className='input'>
                        <p> Price</p>
                        <input type='number' {...register("price", { required: true, minLength: 1, maxLength: 20 })} />
                        <span>
                            {errors.price?.type === "minLength" && "This field must contain 1 character"}
                            {errors.price?.type === "maxLength" && "This field must be less than 20 length"}
                            {errors.price?.type === "required" && "This field Required"}
                        </span>
                    </div>
                    <div className="btn">
