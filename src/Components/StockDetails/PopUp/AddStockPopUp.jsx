...
function AddStockPopUp(props) {
    ...
    const onSubmit = (data) => {
        if (data && stockName.length !== 0) {//connect to server
            axios({
                method: 'PUT',
                url: 'http://localhost:4000/stock/update',
                headers: {
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className='input'>
                        ...
                        <select {...register("stockid", { required: true })} onClick={(e) => {
                            setError1('')
                            setStockName(props.allStockData.filter((data) => String(data.id) === String(e.currentTarget.value)))
                        }}>
                            {
                                props.allStockData.length !== 0 ? props.allStockData.map((data) => {
                                    return (
                                        data.category === null ? <option key={data.id} value={data.id}>{data.name}</option> : null
                                    )
