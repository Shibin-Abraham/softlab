...
function Dump() {
    ...
    let getStockData = useCallback(()=>{
        axios({
            ...
            if(res.data.length !== undefined){
                ...
                setAllStockData(res.data.filter((data)=>data.category!==''))
    ...
    let getDumpItemData = useCallback(()=>{
        axios({
          method: 'POST',
          url: 'http://localhost/soft-lab-api/route/services/dumped-items-data.php',
          headers: {
              'Content-type': 'application/json; charset=utf-8',
              'Authorization': authData.JWT, 
            }
          ...
          if(res.data.length !== undefined){
              ...
              setDumpItemsData(res.data)
              setDumpItemsDataCopy(res.data)
                       ...
                       allStockData.length!==0?allStockData.map((data)=>{
                            ...
                            return(
                                <div className="card" key={data.id} onClick={()=>{
                                    setDumpItemsDataCopy(dumpItemsData.filter((d)=>d.s_id===data.id))
                                }}>
                                    <div className="svg">
                        ...
                        dumpItemsDataCopy.length!==0?dumpItemsDataCopy.map((data,index)=>{
                            return(
                                <div className="card" key={index}>
                                    ...
                                    <h4>{data.name}</h4>
                                    <div className='discription'>
                                        <span>{`${data.b_name.toUpperCase()}, ${data.model}, ${data.description}`}</span>
                                    </div>
